...
long long solve_problem_block_part2(int start_col, int end_col) {
    ...
    for (int c = start_col; c <= end_col; c++) {
        ...
        for (int r = 0; r < max_row; r++) {
            ...
            if (isdigit(ch)) digit_buffer[buf_idx++] = ch;
            else if (ch == '+' || ch == '*') operator = ch;
        }
        digit_buffer[buf_idx] = '\0';

        if (buf_idx > 0) numbers[count++] = atoll(digit_buffer);
    }
...
int main() {
    ...
    for (int c = 0; c <= max_col; c++) {
        ...
        if (!in_block && !empty) {
            ...
            grand_total += solve_problem_block_part2(start_col, c - 1);
            in_block = 0;
        }
    }
